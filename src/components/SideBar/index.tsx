        ...
        <nav className={`relative overflow-y-auto overflow-x-hidden duration-500 bg-palette-menu transition-all px-5 ${open ? "w-60" : "w-24"}`} >
            ...
            <div className={`${!open && 'pt-5'}`}>
                <div className={`${!open ? 'flex flex-col justify-end' : 'pt-3'} `}>
                    ...
                    <div className="">
                        <button className={`text-light border-2 border-palette-gray p-2 flex items-center gap-3 rounded-md justify-center w-full ${!open && 'border-none w-20'}`} onClick={() => {
                            sessionLogout().then(() => {
                                localStorage.removeItem("JWT_TOKEN");
                                localStorage.removeItem("logged_keycloak")
                                signOut({ callbackUrl: "/" });
                            });
                        }}>
